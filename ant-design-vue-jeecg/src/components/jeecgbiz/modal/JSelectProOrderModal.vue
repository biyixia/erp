...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: '件装量',
          key: 'product.unitLoad',
          width: 100,
          dataIndex: ['product', 'unitLoad']
        },
        {
          title: '件数',
          width: 100,
          ...
          customRender(text, row, index) {
            return parseInt(row.produceNumber / row.product.unitLoad);
          }
        },
        {
          title: '散件',
          ...
          dataIndex: 'looseItem',
          customRender(text, row, index) {
            return (row.produceNumber % row.product.unitLoad).toFixed(3);
          }
        },
        {
          title: '数量',
        ...
        },
        {
          ...
          dataIndex: 'amount',
          customRender(text, row, index) {
            return row.product.price * row.produceNumber;
          }
        },
        {
          title: '含税单价',
          key: 'product.priceIncludedTax',
          width: 100,
          dataIndex: ['product', 'priceIncludedTax'],
        },
        {
          title: '含税金额',
          width: 100,
          ...
          customRender(text, row, index) {
            return row.product.priceIncludedTax * row.produceNumber;
          }
        },
        {
          title: '累计入库数量',
  ...
  methods: {
    async loadData(arg) {
      ...
      this.loading = true
      getAction('/bill/bill/referPIS', params).then(res => {
        if (res.success) {
    ...
    getSelectPoRows() {
      ...
      for (let row of this.selectionRows) {
        if (this.selectedRowKeys.includes(row.id)) {
          row.source = '生产订单'
          row.sourceNumber = row.code
          row.referId = row.id
          row.materialCode = row.product.code
          row.count = row.produceNumber
          row.number = parseInt(row.produceNumber / row.product.unitLoad)
          row.looseItem = (row.produceNumber % row.product.unitLoad).toFixed(3)
          row.taxIncludedAmount = row.product.priceIncludedTax * row.produceNumber
          row.amount = row.product.price * row.produceNumber
          row.tax = row.taxIncludedAmount - row.amount
