...
export default {
  ...
  methods: {
    async loadData(arg) {
      ...
      getAction('/bill/bill/referCraftDetailList/proci', params).then(res => {
        if (res.success) {
    ...
    getSelectPoRows() {
      ...
      for (let row of this.selectionRows) {
        if (this.selectedRowKeys.includes(row.id)) {
          ...
          row.referId = row.id
          row.craftAmount = row.bomRouting.processCost * row.completeQuantity
          row.wasteAmount = row.bomRouting.wagePrice * row.wasteQuantity
          row.totalAmount = row.craftAmount - row.wasteAmount
          row.amount = (row.totalAmount/(1+row.product.rate)).toFixed(3)
          row.tax = (row.totalAmount - row.amount).toFixed(3)
          row.priceIncludedTax = row.totalAmount
