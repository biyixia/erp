      ...
      <a-col>
        ...
        <div v-if="actionButton" class="action-button">
          <a-button-group v-if="buttonPermission('add')">
            <a-button v-if="addButton" type="primary" icon="plus" @click="handleClickAdd" :disabled="disabled">新增
            </a-button>
              ...
              >
                ...
                <div
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <label :key="i" v-if="col.type === formTypes.input || col.type === formTypes.inputNumber">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <input
                          ...
                          v-model="inputValues[rowIndex][col.key]"
                          :id="id"
                    ...
                    <!-- 物料选择 -->
                    <template v-else-if="col.type === formTypes.sel_material">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <j-select-material
                          v-if="isEditRow(row, col)"
                          :id="id"
                          ...
                          v-bind="buildProps(row,col)"
                          style="width: 100%;"
                          :value="materialCompValues[id]"
                          :placeholder="replaceProps(col, col.placeholder)"
                          :trigger-change="true"
                          @change="(v)=>handleChangeMaterialCommon(v,id,row,col)"
                          @input="(value,others)=>materialCallback(value,id,row,col,rowIndex)"
                        />
                        <span
                          v-else
                          class="j-td-span no-edit"
                          :class="{disabled: buildProps(row,col).disabled}"
                          @click="handleEditRow(row, col)"
                        >{{ materialCompValues[id] }}</span>
                      </a-tooltip>
                    </template>

                    <!-- 客商选择 -->
                    <template v-else-if="col.type === formTypes.sel_merchant">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <j-select-merchant
                          v-if="isEditRow(row, col)"
                          :id="id"
                        ...
                        <span
                          ...
                          :class="{disabled: buildProps(row,col).disabled}"
                          @click="handleEditRow(row, col)"
                        >{{ merchantCompValues[id] }}</span>
                      </a-tooltip>
                    </template>

                    <!-- 工艺选择 -->
                    <template v-else-if="col.type === formTypes.sel_craft">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <j-select-craft
                          v-if="isEditRow(row, col)"
                          :id="id"
                          ...
                          v-bind="buildProps(row,col)"
                          style="width: 100%;"
                          :value="craftCompValues[id]"
                          :placeholder="replaceProps(col, col.placeholder)"
                          :trigger-change="true"
                          @change="(v)=>handleChangeCraftCommon(v,id,row,col)"
                          @input="(value,others)=>materialCallback(value,id,row,col,rowIndex)"
                        />
                        <span
                          v-else
...
import JSelectMaterial from "@comp/jeecgbiz/JSelectMaterial";
import JSelectMerchant from "@comp/jeecgbiz/JSelectMerchant";
import JSelectCraft from "@comp/jeecgbiz/JSelectCraft";
...
export default {
  name: 'JEditableTable',
  components: {JSelectMerchant, JSelectMaterial, JSelectCraft, JDate, Draggable, JInputPop, JFilePop},
  provide() {
    return {
  ...
  methods: {
    ...
    clearRow() {
      ...
      this.userCompValues = {}
      this.materialCompValues = {}
      this.merchantCompValues = {}
      this.craftCompValues = {}
      this.slotValues = {}
    ...
    _pushByDataSource(dataSource, insertIndexes = null, update = true, rows = null, setDefaultValue = false) {
      ...
      let materialCompValues = {...this.materialCompValues}
      let merchantCompValues = {...this.merchantCompValues}
      let craftCompValues = {...this.craftCompValues}
      ...
      dataSource.forEach((data, newValueIndex) => {
        ...
        this.columns.forEach(column => {
          ...
          let sourceValue = '';
          if (column.type === FormTypes.sel_material && data['product'] !== undefined && column.key !== 'productCode') {
            sourceValue = (String)(data['product'][column.key]);
          } else if (column.type === FormTypes.sel_material && data['material'] !== undefined && column.key !== 'materialCode') {
            sourceValue = (String)(data['material'][column.key]);
          } else if (column.type === FormTypes.sel_merchant && data['merchant'] !== undefined && column.key !== 'clientId') {
            sourceValue = (String)(data['merchant'][column.key]);
          } else if (column.type === FormTypes.sel_craft && data['bomRouting'] !== undefined && column.key !== 'craftCode') {
            if (data['bomRouting'] === null) {
              sourceValue = (String)(data['refer'][column.key]);
            } else {
              sourceValue = (String)(data['bomRouting'][column.key]);
            }
          } else {
            sourceValue = (data[column.key] == null ? '' : data[column.key]).toString();
          }
          ...
          } else if (column.type === FormTypes.sel_material) {
            materialCompValues[inputId] = sourceValue
          } else if (column.type === FormTypes.sel_merchant) {
            merchantCompValues[inputId] = sourceValue
          } else if (column.type === FormTypes.sel_craft) {
            craftCompValues[inputId] = sourceValue
      ...
      this.materialCompValues = materialCompValues
      this.merchantCompValues = merchantCompValues
      this.craftCompValues = craftCompValues
      ...
      for (let value of inputValues) {
        ...
        this.columns.forEach(column => {
          ...
          if (column.type === FormTypes.checkbox) {
            ...
            value[column.key] = this.materialCompValues[inputId]

          } else if (column.type === FormTypes.sel_merchant) {
            value[column.key] = this.merchantCompValues[inputId]
          } else if (column.type === FormTypes.sel_craft) {
            value[column.key] = this.craftCompValues[inputId]
    ...
    _getAllValuesForSlot() {
      return cloneObject({
        ...
        materialCompValues: this.materialCompValues,
        merchantCompValues: this.merchantCompValues,
        craftCompValues: this.craftCompValues,
              ...
              if (column.key === newValueKey) {
                ...
                if (!edited) {
                  ...
                  if (column.type === FormTypes.select) {
                    ...
                    edited = this.setOneValue(this.materialCompValues, modelKey, newValue)
                  } else if (column.type === FormTypes.sel_merchant) {
                    edited = this.setOneValue(this.merchantCompValues, modelKey, newValue)
                  } else if (column.type === FormTypes.sel_craft) {
                    edited = this.setOneValue(this.craftCompValues, modelKey, newValue)
                  } else if (column.type === FormTypes.input_pop || column.type === 'textarea') {
    ...
    recalcOneStatisticsColumn(key) {
      if (this.hasStatisticsColumn) {
        if (this.statisticsColumns.hasOwnProperty(key)) {
          ...
          this.statisticsColumns[key] = count.toFixed(3)
        }
      }
    },
    ...
    handleBlurCommono(target, index, row, column) {
      //自动更新数量、件数、散件
      let unitLoad = this.getValuesSync().values[index]['unitLoad']
      let number = this.inputValues[index]['number']
      let looseItem = (Number)(this.inputValues[index]['looseItem'])
      let count = this.inputValues[index]['count']
      if (unitLoad !== undefined && unitLoad !== '') {
        //填写数量
        if (column.key === 'count') {
          this.inputValues[index]['number'] = parseInt(count / unitLoad);
          this.inputValues[index]['looseItem'] = count % unitLoad;
        }
        //填写件数
        if (column.key === 'number') {
          this.inputValues[index]['count'] = unitLoad * number + looseItem;
        }
        //填写散件
        if (column.key === 'looseItem') {
          this.inputValues[index]['count'] = unitLoad * number + looseItem;
        }
        //自动更新含税金额
        let priceIncludedTax = this.getValuesSync().values[index]['priceIncludedTax'];
        let price = this.getValuesSync().values[index]['price']
        if (priceIncludedTax !== undefined && priceIncludedTax !== '') {
          this.inputValues[index]['taxIncludedAmount'] = (priceIncludedTax * this.inputValues[index]['count']).toString();
        }
        if (price !== undefined && price !== '') {
          this.inputValues[index]['amount'] = (price * this.inputValues[index]['count']).toFixed(3).toString();
        }
        this.inputValues[index]['tax'] = (this.inputValues[index]['taxIncludedAmount'] - this.inputValues[index]['amount']).toFixed(3).toString()
      }
      //自动更新加工金额和废品金额
      let processCost = this.getValuesSync().values[index]['processCost']
      let completeQuantity = this.inputValues[index]['completeQuantity']
      let wagePrice = this.getValuesSync().values[index]['wagePrice']
      let wasteQuantity = this.inputValues[index]['wasteQuantity']
      if (processCost !== undefined && processCost !== '') {
        //填写加工数量
        if (column.key === 'completeQuantity') {
          this.inputValues[index]['craftAmount'] = processCost * completeQuantity
          this.inputValues[index]['totalAmount'] = (processCost * completeQuantity) - (wagePrice * wasteQuantity)
        }
      }
      if (wagePrice !== undefined && wagePrice !== '') {
        //填写废品数量
        if (column.key === 'wasteQuantity') {
          this.inputValues[index]['wasteAmount'] = wagePrice * wasteQuantity
          this.inputValues[index]['totalAmount'] = (processCost * completeQuantity) - (wagePrice * wasteQuantity)
        }
      }
      let rate = (Number)(this.inputValues[index]['rate'])
      //填写税率修改无税金额和税额
      if (column.key === 'rate') {
        this.inputValues[index]['amount'] = (this.inputValues[index]['totalAmount'] / (1 + rate)).toFixed(3)
        this.inputValues[index]['tax'] = (this.inputValues[index]['totalAmount'] - this.inputValues[index]['amount']).toFixed(3)
      }

      if (this.inputValues[index]['payRatio']) {
        this.inputValues[index]['payAmount'] = this.inputValues[index]['payRatio'] / 100 * this.taxIncludedAmount;
      }

      this.recalcOneStatisticsColumn('taxIncludedAmount')
      this.recalcOneStatisticsColumn('amount')
      this.recalcOneStatisticsColumn('tax')
      this.recalcOneStatisticsColumn('payAmount')
      this.$emit('blurInputValues',this.inputValues)
    ...
    //物料组件值改变
    handleChangeMaterialCommon(value, id, row, column) {
      this.materialCompValues = this.bindValuesChange(value, id, 'materialCompValues')
      this.validateOneInput(value, row, column, this.notPassedIds, true, 'change')
      // 触发valueChange 事件
      this.elemValueChange(FormTypes.sel_material, row, column, value)
    },
    //客商组件值改变
    handleChangeMerchantCommon(value, id, row, column) {
      this.merchantCompValues = this.bindValuesChange(value, id, 'merchantCompValues')
      this.validateOneInput(value, row, column, this.notPassedIds, true, 'change')
      // 触发valueChange 事件
      this.elemValueChange(FormTypes.sel_merchant, row, column, value)
    },    //客商组件值改变
    handleChangeCraftCommon(value, id, row, column) {
      this.craftCompValues = this.bindValuesChange(value, id, 'craftCompValues')
      this.validateOneInput(value, row, column, this.notPassedIds, true, 'change')
      // 触发valueChange 事件
      this.elemValueChange(FormTypes.sel_craft, row, column, value)
    },
    ...
    buildTdStyle(col) {
      ...
      if (col.type === FormTypes.sel_material && !col.width) {
        style['width'] = '220px'
      }
    ...
    buildProps(row, col) {
      ...
      if (col.type === FormTypes.sel_depart || col.type === FormTypes.sel_user || col.type === FormTypes.sel_material || col.type === FormTypes.sel_craft ||
        (col.type === FormTypes.sel_merchant && col.key !== 'clientId')
      ) {
    ...
    getStoreAndTextField(col) {
      ...
      if (col.type === FormTypes.sel_depart) {
        ...
        storeField = 'username'
        textField = 'realname'
      } else if (col.type === FormTypes.sel_material || col.type === FormTypes.sel_craft || (col.type === FormTypes.sel_merchant && col.key !== 'clientId')) {
        storeField = col.key
        textField = col.key
      }
    ...
    /** 物料选择框回调 */
    materialCallback(value, id, row, column, index) {
      let rowKey = this.getCleanId(row.id)
      let setValueItem = {rowKey, values: {}}
      Object.keys(value).forEach(key => {
        if (value[key] != null) {
          setValueItem.values[key] = value[key].toString();
          if (key === 'code') {
            setValueItem.values['materialCode'] = value['code'].toString();
          }
        } else {
      ...
      this.setOneValue(this.materialCompValues, id, value)
      // 做单个表单验证
      this.validateOneInput(value, row, column, this.notPassedIds, true, 'change')
      // 触发valueChange 事件
      this.elemValueChange(FormTypes.sel_material, row, column, value)
