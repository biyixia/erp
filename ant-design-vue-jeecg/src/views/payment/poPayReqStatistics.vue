    ...
    <div class="table-page-search-wrapper">
      ...
      <a-form layout="inline" @keyup.enter.native="searchQuery">
        <a-row :gutter="24">
          ...
          <a-col :md="6" :sm="24">
            <a-form-item label="未付款" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
              <j-dict-select-tag type="list" v-model="queryParam.noPay" dictCode="is_open"
                                 placeholder="是否未付款"/>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="24">
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          title: '累计付款金额',
          width: 120,
          dataIndex: 'sumApplyAmount',
        },
        {
          title: '未付款金额',
          width: 120,
          dataIndex: 'sumNoApplyAmount',
          customRender(text, row, index) {
            return row.applyAmount - row.sumApplyAmount;
          }
        },
        {
          title: '预计付款日期',
  ...
  methods: {
    async loadData(arg) {
      ...
      this.loading = true
      params.total = true;
      getAction('/bill/bill/referPayDetailList/pp', params).then(res => {
        if (res.success) {
