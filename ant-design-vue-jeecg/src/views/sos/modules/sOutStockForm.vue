  ...
  <a-spin :spinning="confirmLoading">
        ...
        <a-row>
          ...
          <a-col :xs="24" :sm="12">
            <a-form-model-item label="调出仓库" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="outWarehouseId">
              <j-select-warehouse ownership="company" v-model="model.outWarehouseId" :multi="false"/>
            </a-form-model-item>
          </a-col>
          <a-col :xs="24" :sm="12">
    ...
    <a-tabs v-model="activeKey" @change="handleChangeTabs">
      <a-tab-pane tab="出库明细" :key="refKeys[0]" :forceRender="true">
        <j-editable-table
          :warehouse="model.outWarehouseId"
          :ref="refKeys[0]"
          ...
          <template slot="buttonAfter">
            <a-col>
              <a-button type="primary" @click="getReferPoData" :disabled="formDisabled">参照销售订单</a-button>
            </a-col>
          </template>
        </j-editable-table>
      </a-tab-pane>
    ...
    <j-select-so-modal :client-id="model.clientId" @ok="selectOK" ref="selectSo"/>
  </a-spin>
</template>
...
import JSelectSoModal from '@/components/jeecgbiz/modal/JSelectSoModal'
...
export default {
  ...
  data() {
    return {
                  ...
                  if (typeof value === 'string') {
                    ...
                    if (values[0].standardDosage === '') {
                      getAction('/initial/material/list', params).then(res => {
                        if (res.success) {
                          if (res.result.records[0].isOutContract === 'Y') {
                            callback(false, '该物料只能根据销售订单录入')
                            return
                          }
                          callback(true);
                        }
              ...
              handler(type, value, row, column, callback, target) {
                if (type === 'getValues' || type === 'blur') {
                  ...
                  if (values[0].standardDosage !== '') {
                    getAction('/initial/material/list', params).then(res => {
                      if (res.success) {
                        if (res.result.records[0].isBuyorderOut === 'N' && values[0].count > values[0].standardDosage) {
                          callback(false, '该物料不允许超过销售订单的销售数量出库')
                          return
                        }
                        callback(true);
                      }
      ...
      url: {
        add: "/bill/bill/add/sos",
        edit: "/bill/bill/edit",
        billingDetail: {
          list: '/bill/bill/queryBillingDetailByMainId'
        },
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/sell_out_stock_num'
        },
      }
    }
  },
