...
<!--      <j-super-query :fieldList="superFieldList" ref="superQueryModal" @handleSuperQuery="handleSuperQuery"></j-super-query>-->
      <a-dropdown v-if="selectedRowKeys.length > 0">
        <a-menu slot="overlay">
          ...
          <a-menu-item key="2" v-has="'sos:check'" @click="batchSetStatus(1)"><a-icon type="check"/>审核</a-menu-item>
          <a-menu-item key="3" v-has="'sos:check'" @click="batchSetStatus(0)"><a-icon type="stop"/>反审核</a-menu-item>
        </a-menu>
        <a-button>
          <span>批量操作</span>
          <a-icon type="down"/>
      ...
      >
        ...
        <template slot="expandedRowRender" slot-scope="record">
          <a-tabs tabPosition="top">
            <a-tab-pane tab="出库明细" key="billingDetail" forceRender>
        ...
        <template slot="customRenderStatus" slot-scope="status">
          ...
          <a-tag v-if="status === '2'" color="cyan">部分申请开票</a-tag>
          <a-tag v-if="status === '3'" color="blue">全部申请开票</a-tag>
        </template>
    ...
    <!-- 表单区域 -->
    <s-out-stock-modal ref="modalForm" @ok="modalFormOk"/>
...
<script>
  ...
  import sOutStockModal from './modules/sOutStockModal'
  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            title: '调出仓库',
            align: 'center',
            key:'outWarehouse.warehouseName',
            dataIndex: ['outWarehouse','warehouseName'],
          },
          {
            title: '备注',
            align: 'center',
    ...
    methods: {
      ...
      getSuperFieldList(){
        ...
        fieldList.push({type:'popup',value:'clientName',text:'客户名称', popup:{code:'byx_merchant',field:'name',orgFields:'name',destFields:'client_name'}})
        fieldList.push({type:'popup',value:'warehouseName',text:'仓库名称', popup:{code:'byx_warehouse',field:'name',orgFields:'name',destFields:'warehouse_name'}})
