...
<!--      <a-button type="primary" icon="download" @click="handleExportXls('补料申请单')">导出</a-button>-->
...
<!--      <j-super-query :fieldList="superFieldList" ref="superQueryModal" @handleSuperQuery="handleSuperQuery"></j-super-query>-->
      <a-dropdown v-if="selectedRowKeys.length > 0">
        <a-menu slot="overlay">
          ...
          <a-menu-item key="2" v-has="'rr:check'" @click="batchSetStatus(1)"><a-icon type="check"/>审核</a-menu-item>
          <a-menu-item key="3" v-has="'rr:check'" @click="batchSetStatus(0)"><a-icon type="stop"/>反审核</a-menu-item>
        </a-menu>
        <a-button>
          <span>批量操作</span>
          <a-icon type="down"/>
      ...
      >
        <template slot="expandedRowRender" slot-scope="record">
          <a-tabs tabPosition="top">
            <a-tab-pane tab="申请明细" key="billingDetail" forceRender>
        ...
        <template slot="customRenderStatus" slot-scope="status">
          ...
          <a-tag v-if="status === '2'" color="cyan">部分下单</a-tag>
          <a-tag v-if="status === '3'" color="blue">全部下单</a-tag>
        </template>
    ...
    <!-- table区域 end -->
    <!-- 表单区域 -->
    <replenish-req-modal ref="modalForm" @ok="modalFormOk"/>
  </a-card>
</template>
<script>
  import { JeecgListMixin } from '@/mixins/JeecgListMixin'
  import replenishReqModal from './modules/replenishReqModal'
  ...
  export default {
    ...
    data() {
      return {
        ...
        expandedRowKeys: [],
        url: {
          list: '/bill/bill/list/rr',
          delete: '/bill/bill/delete',
          deleteBatch: '/bill/bill/deleteBatch',
          exportXlsUrl: '/bill/bill/exportXls/rr',
