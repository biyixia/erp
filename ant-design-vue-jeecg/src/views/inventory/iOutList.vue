<template>
  <a-card class="j-inner-table-wrapper" :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('调拨出库单')">导出</a-button>
...
<!--      <j-super-query :fieldList="superFieldList" ref="superQueryModal" @handleSuperQuery="handleSuperQuery"></j-super-query>-->
      <a-dropdown v-if="selectedRowKeys.length > 0">
        <a-menu slot="overlay">
          ...
          <a-menu-item key="2" v-has="'io:check'" @click="batchSetStatus(1)"><a-icon type="check"/>审核</a-menu-item>
          <a-menu-item key="3" v-has="'io:check'" @click="batchSetStatus(0)"><a-icon type="stop"/>反审核</a-menu-item>
        </a-menu>
        <a-button>
          <span>批量操作</span>
          <a-icon type="down"/>
    ...
    <div>
      ...
      >
        ...
        <template slot="customRenderStatus" slot-scope="status">
          ...
          <a-tag v-if="status === '2'" color="cyan">部分入库</a-tag>
          <a-tag v-if="status === '3'" color="blue">全部入库</a-tag>
        </template>
    ...
    <!-- table区域 end -->
    <!-- 表单区域 -->
    <i-out-modal ref="modalForm" @ok="modalFormOk"/>
  </a-card>
</template>
<script>
  import { JeecgListMixin } from '@/mixins/JeecgListMixin'
  import iOutModal from './modules/iOutModal'
  ...
  export default {
    ...
    data() {
      return {
        ...
        expandedRowKeys: [],
        url: {
          list: '/bill/bill/list/io',
          delete: '/bill/bill/delete',
          deleteBatch: '/bill/bill/deleteBatch',
          exportXlsUrl: '/bill/bill/exportXls/ao',
    ...
    methods: {
      ...
      getSuperFieldList(){
        ...
        fieldList.push({type:'popup',value:'warehouseName',text:'调出仓库', popup:{code:'byx_warehouse',field:'warehouse_name',orgFields:'warehouse_name',destFields:'warehouse_name'}})
