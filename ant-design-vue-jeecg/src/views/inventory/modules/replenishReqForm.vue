    ...
    <j-form-container :disabled="formDisabled">
      ...
      <a-form-model ref="form" :model="model" :rules="validatorRules" slot="detail">
        <a-row>
          ...
          <a-col :xs="24" :sm="12">
            <a-form-model-item label="需求日期" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="demandedDate">
      ...
      <a-tab-pane tab="申请明细" :key="refKeys[0]" :forceRender="true">
        <j-editable-table
          ...
          <template slot="buttonAfter">
            <a-col>
              <a-button type="primary" @click="runROP" :disabled="formDisabled">ROP运算</a-button>
            </a-col>
          </template>
        </j-editable-table>
      </a-tab-pane>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        add: "/bill/bill/add/rr",
        edit: "/bill/bill/edit",
        billingDetail: {
          list: '/bill/bill/queryBillingDetailByMainId'
        },
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/rr_num'
        },
      }
    }
  },
  ...
  methods: {
    runROP(){
      this.loading = true
      getAction('/bill/bill/getRop').then(res => {
        console.log(res)
        if (res.success) {
          for (let row of res.result.records) {
            ...
            setValueItem.values['number'] = parseInt(row['count'] / setValueItem.values['unitLoad']);
            setValueItem.values['looseItem'] = row['count'] % setValueItem.values['unitLoad'];
            setValueItem.values['source'] = 'ROP运算';
            setValueItem.values['taxIncludedAmount'] = (setValueItem.values['priceIncludedTax'] * row['count']).toFixed(3);
            setValueItem.values['amount'] = (setValueItem.values['price'] * row['count']).toFixed(3);
            setValueItem.values['tax'] = (setValueItem.values['taxIncludedAmount'] - setValueItem.values['amount']).toFixed(3);
