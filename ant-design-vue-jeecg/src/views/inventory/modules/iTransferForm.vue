<template>
  <a-spin :spinning="confirmLoading">
        ...
        <a-row>
          ...
          <a-col :xs="24" :sm="12">
            <a-form-model-item label="调入仓库" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="inWarehouseId">
              <j-select-warehouse v-model="model.inWarehouseId" :multi="false"/>
            </a-form-model-item>
          </a-col>
          <a-col :xs="24" :sm="12">
    ...
    <a-tabs v-model="activeKey" @change="handleChangeTabs">
      <a-tab-pane tab="调拨明细" :key="refKeys[0]" :forceRender="true">
        <j-editable-table
          :ref="refKeys[0]"
  ...
  data() {
    const checkWarehouse = (rule, value, callback) => {
      if (!value) {	//如果输入为空直接返回
        ...
        if (this.model.inWarehouseId !== this.model.outWarehouseId) {	//匹配成功返回
          callback();
        } else {	//匹配不成功返回错误显示
          callback(new Error("调入仓库与调出仓库不能为同一仓库！"));
        }
      }
    };
    return {
      ...
      url: {
        add: "/bill/bill/add/it",
        edit: "/bill/bill/edit",
        billingDetail: {
          list: '/bill/bill/queryBillingDetailByMainId'
        },
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/inventory_transfer_num'
        },
      }
    }
  },
