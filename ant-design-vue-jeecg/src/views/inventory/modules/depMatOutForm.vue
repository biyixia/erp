<template>
  <a-spin :spinning="confirmLoading">
        ...
        <a-row>
          ...
          <a-col :xs="24" :sm="12">
            <a-form-model-item label="调出仓库" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="outWarehouseId">
              <j-select-warehouse ownership="company" v-model="model.outWarehouseId"/>
            </a-form-model-item>
          </a-col>
          <a-col :xs="24" :sm="12">
      ...
      <a-tab-pane tab="出库明细" :key="refKeys[0]" :forceRender="true">
        <j-editable-table
          ...
          <template slot="buttonAfter">
            <a-col>
              <a-button type="primary" @click="getReferDMRData" :disabled="formDisabled">参照部门领料申请单</a-button>
            </a-col>
          </template>
        </j-editable-table>
      </a-tab-pane>
    ...
    <j-select-d-m-r-modal @ok="selectOK" ref="selectDMR"/>
  </a-spin>
</template>
...
import JSelectDMRModal from '@/components/jeecgbiz/modal/JSelectDMRModal'
import {validateDuplicateValue} from '@/utils/util'
import {getAction, putAction} from "@api/manage";
import JSelectWarehouse from "@comp/jeecgbiz/JSelectWarehouse";
...
export default {
  ...
  data() {
    return {
                  ...
                  if (typeof value === 'string') {
                    ...
                    if (values[0].standardDosage === '') {
                      getAction('/initial/material/list', params).then(res => {
                        if (res.success) {
                          if (res.result.records[0].isApply === 'Y') {
                            callback(false, '该物料只能根据领料申请录入')
                            return
                          }
                          callback(true);
                        }
              ...
              handler(type, value, row, column, callback, target) {
                if (type === 'change' || type === 'getValues') {
                  ...
                  if (values[0].standardDosage !== '') {
                    getAction('/initial/material/list', params).then(res => {
                      if (res.success) {
                        if (res.result.records[0].isApplyIn === 'N' && values[0].count > values[0].standardDosage) {
                          callback(false, '该物料不允许超过领料申请的数量出库')
                          return
                        }
                        callback(true);
                      }
      ...
      url: {
        add: "/bill/bill/add/dmo",
        edit: "/bill/bill/edit",
        billingDetail: {
          list: '/bill/bill/queryBillingDetailByMainId'
        },
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/dmo_num'
        },
      }
    }
  },
