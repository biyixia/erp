    ...
    <div class="table-page-search-wrapper">
      ...
      <a-form layout="inline" @keyup.enter.native="searchQuery">
        <a-row :gutter="24">
          ...
          <a-col :md="6" :sm="24">
            <a-form-model-item label="调入仓库" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
              <j-select-warehouse v-model="queryParam.inWarehouseId" :multi="false"/>
            </a-form-model-item>
          </a-col>
          <a-col :md="6" :sm="24">
            <a-form-model-item label="调出仓库" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
              <j-select-warehouse v-model="queryParam.outWarehouseId" :multi="false"/>
            </a-form-model-item>
          </a-col>
          <a-col :md="6" :sm="24">
        ...
        <a-row :gutter="24">
          ...
          <a-col :md="6" :sm="24">
            <a-form-item label="未入库　" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
              <j-dict-select-tag type="list" v-model="queryParam.noInBound" dictCode="is_open"
                                 placeholder="是否未入库"/>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="24">
            <a-form-item label="未出库　" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
              <j-dict-select-tag type="list" v-model="queryParam.noOutBound" dictCode="is_open"
                                 placeholder="是否未出库"/>
            </a-form-item>
          </a-col>
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          title: '未入库数量',
          width: 120,
          dataIndex: 'sumNoInBound',
          customRender(text, row, index) {
            return row.count - row.sumInBound
          }
        },
        {
          title: '累计出库数量',
  ...
  methods: {
    async loadData(arg) {
      ...
      this.loading = true
      params.total = true
      getAction('/bill/bill/referPoList/it', params).then(res => {
        if (res.success) {
    ...
    getQueryParams() {
      //获取查询条件
      let sqp = {}
      this.queryParam.inWarehouseId = this.inWarehouseId;
      this.queryParam.outWarehouseId = this.outWarehouseId;
      console.log(this.outWarehouseId)
