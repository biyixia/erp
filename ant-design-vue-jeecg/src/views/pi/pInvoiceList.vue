<template>
  <a-card class="j-inner-table-wrapper" :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('采购发票')">导出</a-button>
...
<!--      <j-super-query :fieldList="superFieldList" ref="superQueryModal" @handleSuperQuery="handleSuperQuery"></j-super-query>-->
      <a-dropdown v-if="selectedRowKeys.length > 0">
        <a-menu slot="overlay">
          ...
          <a-menu-item key="2" v-has="'pi:check'" @click="batchSetStatus(1)"><a-icon type="check"/>审核</a-menu-item>
          <a-menu-item key="3" v-has="'pi:check'" @click="batchSetStatus(0)"><a-icon type="stop"/>反审核</a-menu-item>
        </a-menu>
        <a-button>
          <span>批量操作</span>
          <a-icon type="down"/>
    ...
    <div>
      ...
      >
        ...
        <template slot="customRenderStatus" slot-scope="status">
          ...
          <a-tag v-if="status === '2'" color="cyan">部分申请付款</a-tag>
          <a-tag v-if="status === '3'" color="blue">全部申请付款</a-tag>
        </template>
    ...
    <!-- 表单区域 -->
    <p-invoice-modal ref="modalForm" @ok="modalFormOk"/>
...
<script>
  ...
  import pInvoiceModal from './modules/pInvoiceModal'
