<template>
  <a-spin :spinning="confirmLoading">
        ...
        <a-row>
          ...
          <a-col :xs="24" :sm="12">
            <a-form-model-item label="发票类别" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="invoiceType">
              <j-dict-select-tag type="list" v-model="model.invoiceType" dictCode="invoice_type"/>
            </a-form-model-item>
          </a-col>
          <a-col :xs="24" :sm="12">
            <a-form-model-item label="发票号码" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="invoiceNumber">
              <a-input v-model="model.invoiceNumber"/>
            </a-form-model-item>
          </a-col>
          <a-col :xs="24" :sm="12">
      ...
      <a-tab-pane tab="发票明细" :key="refKeys[0]" :forceRender="true">
        ...
        :add-button="false">
          <template slot="buttonAfter">
            <a-button type="primary" @click="getReferPoData" :disabled="formDisabled">参照采购入库单</a-button>
            <span data-v-2f0c1ef1="" class="gap"></span>
            <a-button type="primary" @click="getReferProMatOut" :disabled="formDisabled">参照生产领料出库单</a-button>
          </template>
        </j-editable-table>
      </a-tab-pane>
    ...
    <j-select-p-i-s-modal :client-id="model.clientId" @ok="selectOK" ref="selectPo"/>
    <j-select-pro-mat-out-modal :client-id="model.clientId" @ok="selectOK" ref="selectProMatOut"/>
  </a-spin>
</template>
...
import JSelectPISModal from '@/components/jeecgbiz/modal/JSelectPISModal'
import JSelectProMatOutModal from '@/components/jeecgbiz/modal/JSelectProMatOutModal'
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        add: "/bill/bill/add/pi",
        edit: "/bill/bill/edit",
        billingDetail: {
          list: '/bill/bill/queryBillingDetailByMainId'
        },
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/purchase_invoice_num'
        },
      }
    }
  },
  ...
  methods: {
    ...
    getReferProMatOut(){
      this.$refs.selectProMatOut.showModal()
    },
    getReferPoData() {
      if (this.model.clientId === undefined) {
