<template>
  <a-spin :spinning="confirmLoading">
    ...
    <a-tabs v-model="activeKey" @change="handleChangeTabs">
      <a-tab-pane tab="收款明细" :key="refKeys[0]" :forceRender="true">
        <j-editable-table
          :ref="refKeys[0]"
          ...
          <template slot="buttonAfter">
            <a-col>
              <a-button type="primary" @click="getReferSi" :disabled="formDisabled">参照销售开票申请</a-button>
              <span data-v-2f0c1ef1="" class="gap"></span>
              <a-button type="primary" @click="getReferSoCollect" :disabled="formDisabled">参照销售订单</a-button>
            </a-col>
          </template>
        </j-editable-table>
      </a-tab-pane>
    ...
    <j-select-si-modal @ok="selectOK" ref="selectSi"/>
    <j-select-so-collect-modal @ok="selectOK" ref="selectSo"/>
  </a-spin>
</template>
...
import JSelectSiModal from '@/components/jeecgbiz/modal/JSelectSiModal'
import JSelectSoCollectModal from '@/components/jeecgbiz/modal/JSelectSoCollectModal'
...
export default {
  ...
  components: {JSelectSiModal,JSelectSoCollectModal},
  data() {
    return {
      labelCol: {
        xs: {span: 24},
      ...
      url: {
        add: "/bill/bill/add/collect",
        edit: "/bill/bill/edit",
        payDetail: {
          list: '/bill/bill/queryPayDetailByMainId'
        },
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/sell_invoice_num'
        },
      }
    }
  },
  ...
  methods: {
    ...
    getReferSi(){
      this.$refs.selectSi.showModal()
    },
    getReferSoCollect(){
      this.$refs.selectSo.showModal()
    },
    addBefore() {
