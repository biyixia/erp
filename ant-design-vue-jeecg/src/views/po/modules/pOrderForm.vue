<template>
  <a-spin :spinning="confirmLoading">
    <j-form-container :disabled="formDisabled">
      <a-form-model ref="form" :model="model" :rules="validatorRules" slot="detail">
        <a-row>
          ...
          <a-col :xs="24" :sm="12">
            <a-form-model-item label="付款政策" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="paymentStyle">
      ...
      <a-tab-pane tab="标的明细" :key="refKeys[0]" :forceRender="true">
        <j-editable-table
          ...
          <template slot="buttonAfter">
            <a-col>
              <a-button type="primary" @click="getReferRR" :disabled="formDisabled">参照补料申请单</a-button>
              <span data-v-2f0c1ef1="" class="gap"></span>
              <a-button type="primary" @click="getReferMRP" :disabled="formDisabled">参照MRP计划</a-button>
            </a-col>
          </template>
        </j-editable-table>
      </a-tab-pane>
      <a-tab-pane tab="付款计划" :key="refKeys[1]" :forceRender="true">
        <j-editable-table
          :ref="refKeys[1]"
    ...
    <j-select-r-r-modal @ok="selectOK" ref="selectPo"/>
    <j-select-m-r-p-modal @ok="selectOK" ref="selectMRP"/>
  </a-spin>
</template>
<script>
...
import JSelectRRModal from '@comp/jeecgbiz/modal/JSelectRRModal'
import JSelectMRPModal from '@comp/jeecgbiz/modal/JSelectMRPModal'
  ...
  data() {
    return {
                ...
                if (type === 'change' || type === 'getValues') {
                  ...
                  if (values[0].standardDosage !== '') {
                    getAction('/initial/material/list', params).then(res => {
                      if (res.success) {
                        if (res.result.records[0].isPlanIn === 'N' && values[0].count > values[0].standardDosage) {
                          callback(false, '该物料不允许超过MRP计划的数量采购')
                          return
                        }
                        callback(true);
                      }
            ...
            validateRules: [{ // 自定义函数校验 handler
              handler(type, value, row, column, callback, target) {
                ...
                if (type === 'blur' || type === 'getValues') {
                  let ratio = (Number)(target.getStatisticsValue('payRatio'));
                  if (ratio !== 100.000) {
      ...
      url: {
        ...
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/purchase_order_num'
        },
      }
    }
  },
