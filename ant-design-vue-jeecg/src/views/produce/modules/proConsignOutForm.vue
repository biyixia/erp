<template>
  <a-spin :spinning="confirmLoading">
      ...
      <a-tab-pane tab="加工明细" :key="refKeys[0]" :forceRender="true">
        ...
        >
          <template slot="buttonAfter">
            <a-col>
              <a-button type="primary" @click="referProOrder">参照生产订单</a-button>
            </a-col>
          </template>
        </j-editable-table>
      </a-tab-pane>
    ...
    <j-select-pro-craft-modal consign="1" @ok="selectOK" ref="selectPro"/>
  </a-spin>
</template>
...
import JSelectProCraftModal from '@/components/jeecgbiz/modal/JSelectProCraftModal'
import {validateDuplicateValue} from '@/utils/util'
import {putAction} from "@api/manage";

export default {
  name: 'proConsignOutForm',
  mixins: [JEditableTableModelMixin],
  components: {
    JSelectMerchant,
    JSelectProCraftModal
  },
  data() {
    return {
      labelCol: {
      ...
      url: {
        add: "/bill/bill/add/proco",
        edit: "/bill/bill/edit",
        craftDetail: {
          list: '/bill/bill/queryCraftDetailByMainId'
        },
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/pcol_num'
        },
      }
    }
  },
    ...
    selectOK(selectPoRows) {
      for (let row of selectPoRows) {
        ...
        Object.keys(row).forEach(key => {
          setValueItem.values['productCode'] = row['bill']['product']['code']
          setValueItem.values['materialName'] = row['bill']['product']['materialName']
          setValueItem.values['size'] = row['bill']['product']['size']
