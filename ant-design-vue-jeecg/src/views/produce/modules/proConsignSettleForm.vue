<template>
  <a-spin :spinning="confirmLoading">
      ...
      <a-tab-pane tab="加工明细" :key="refKeys[0]" :forceRender="true">
        ...
        >
          <template slot="buttonAfter">
            <a-col>
              <a-button type="primary" @click="referProci">参照生产委托入库单</a-button>
            </a-col>
          </template>
        </j-editable-table>
      </a-tab-pane>
    ...
    <j-select-proci-modal :client-id="model.clientId"  @ok="selectOK" ref="selectPro"/>
  </a-spin>
</template>
<script>
...
import JSelectProciModal from '@/components/jeecgbiz/modal/JSelectProciModal'
import {validateDuplicateValue} from '@/utils/util'
import {putAction} from "@api/manage";

export default {
  name: 'proConsignSettleForm',
  ...
  data() {
    return {
      ...
      url: {
        add: "/bill/bill/add/procs",
        edit: "/bill/bill/edit",
        craftDetail: {
          list: '/bill/bill/queryCraftDetailByMainId'
        },
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/pcsl_num'
        },
      }
    }
  },
