<template>
  <a-spin :spinning="confirmLoading">
      ...
      <a-tab-pane tab="付款明细" :key="refKeys[0]" :forceRender="true">
        <j-editable-table
          ...
          <template slot="buttonAfter">
            <a-col>
              <a-button type="primary" @click="referProci">参照生产委托结算单</a-button>
            </a-col>
          </template>
        </j-editable-table>
      </a-tab-pane>
    ...
    <j-select-procs-modal @ok="selectOK" ref="selectPro"/>
  </a-spin>
</template>
...
import JSelectProcsModal from '@/components/jeecgbiz/modal/JSelectProcsModal'

import {validateDuplicateValue} from '@/utils/util'
import {putAction} from "@api/manage";

export default {
  name: 'proConsignPayReqForm',
  ...
  data() {
    return {
      ...
      url: {
        add: "/bill/bill/add/procpr",
        edit: "/bill/bill/edit",
        payDetail: {
          list: '/bill/bill/queryPayDetailByMainId'
        },
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/pcprl_num'
        },
      }
    }
  },
