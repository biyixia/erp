<template>
  <a-spin :spinning="confirmLoading">
      ...
      <a-tab-pane tab="标的明细" :key="refKeys[0]" :forceRender="true">
        ...
        >
          <template slot="buttonAfter">
            <a-col>
              <a-button type="primary" @click="getReferProOrder" :disabled="formDisabled">参照生产订单</a-button>
            </a-col>
          </template>
        </j-editable-table>
      </a-tab-pane>
    ...
    <j-select-pro-order-modal @ok="selectOK" ref="selectProOrder"/>
  </a-spin>
</template>
...
import JSelectProOrderModal from '@comp/jeecgbiz/modal/JSelectProOrderModal'
...
export default {
  ...
  data() {
    return {
                ...
                if (type === 'change' || type === 'getValues') {
                  ...
                  if (values[0].standardDosage !== '') {
                    getAction('/initial/material/list', params).then(res => {
                      if (res.success) {
                        if (res.result.records[0].isProductIn === 'N' && values[0].count > values[0].standardDosage) {
                          callback(false, '该物料不允许超过生产订单的生产数量入库')
                          return
                        }
                        callback(true);
                      }
      ...
      url: {
        add: "/bill/bill/add/proi",
        edit: "/bill/bill/edit",
        billingDetail: {
          list: '/bill/bill/queryBillingDetailByMainId'
        },
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/pil_num'
        },
      }
    }
  },
  ...
  methods: {
    ...
    getReferProOrder(){
      this.$refs.selectProOrder.showModal()
    },
    getOrderNum() {
