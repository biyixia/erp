  ...
  data() {
    return {
            ...
            validateRules: [{ // 自定义函数校验 handler
              handler(type, value, row, column, callback, target) {
                ...
                if (type === 'blur' || type === 'getValues') {
                  if (target.getStatisticsValue('payRatio') !== 100) {
      ...
      url: {
        ...
        rule: {
          orderNum: '/sys/fillRule/executeRuleByCode/pmrl_num'
        },
      }
    }
  },
  ...
  methods: {
    ...
    handleChangeTabs(key) {
      if (key === 'collectPayPlan') {
        ...
        if (taxIncludedAmount === 0 || taxIncludedAmount === null) {
