<template>
  <a-card class="j-inner-table-wrapper" :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('生产委托出库单')">导出</a-button>
...
<!--      <j-super-query :fieldList="superFieldList" ref="superQueryModal" @handleSuperQuery="handleSuperQuery"></j-super-query>-->
      <a-dropdown v-if="selectedRowKeys.length > 0">
        <a-menu slot="overlay">
          ...
          <a-menu-item key="2" v-has="'pco:check'" @click="batchSetStatus(1)"><a-icon type="check"/>审核</a-menu-item>
          <a-menu-item key="3" v-has="'pco:check'" @click="batchSetStatus(0)"><a-icon type="stop"/>反审核</a-menu-item>
        </a-menu>
        <a-button>
          <span>批量操作</span>
          <a-icon type="down"/>
      ...
      >
        <template slot="expandedRowRender" slot-scope="record">
          <a-tabs tabPosition="top">
            <a-tab-pane tab="委外明细" key="billingDetail" forceRender>
    ...
    <pro-consign-out-modal ref="modalForm" @ok="modalFormOk"/>
  </a-card>
</template>
<script>

  import { JeecgListMixin } from '@/mixins/JeecgListMixin'
  import proConsignOutModal from './modules/proConsignOutModal'
  import CraftDetailSubTable from './subTables/procoSubTable'
  import {filterMultiDictText} from '@/components/dict/JDictSelectUtil'
  import '@/assets/less/TableExpand.less'
  import {getAction} from "@api/manage";

  export default {
    name: 'proConsignOutList',
