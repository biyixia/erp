<template>
  <a-card class="j-inner-table-wrapper" :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('生产委托结算单')">导出</a-button>
...
<!--      <j-super-query :fieldList="superFieldList" ref="superQueryModal" @handleSuperQuery="handleSuperQuery"></j-super-query>-->
      <a-dropdown v-if="selectedRowKeys.length > 0">
        <a-menu slot="overlay">
          ...
          <a-menu-item key="2" v-has="'pcs:check'" @click="batchSetStatus(1)"><a-icon type="check"/>审核</a-menu-item>
          <a-menu-item key="3" v-has="'pcs:check'" @click="batchSetStatus(0)"><a-icon type="stop"/>反审核</a-menu-item>
        </a-menu>
        <a-button>
          <span>批量操作</span>
          <a-icon type="down"/>
      ...
      >
        <template slot="expandedRowRender" slot-scope="record">
          <a-tabs tabPosition="top">
            <a-tab-pane tab="结算明细" key="billingDetail" forceRender>
              <craft-detail-sub-table :record="record"/>
            </a-tab-pane>
          </a-tabs>
        </template>
        <template slot="customRenderStatus" slot-scope="status">
          ...
          <a-tag v-if="status === '2'" color="cyan">部分申请</a-tag>
          <a-tag v-if="status === '3'" color="blue">全部申请</a-tag>
        </template>
    ...
    <pro-consign-settle-modal ref="modalForm" @ok="modalFormOk"/>
  </a-card>
</template>
<script>

  import { JeecgListMixin } from '@/mixins/JeecgListMixin'
  import proConsignSettleModal from './modules/proConsignSettleModal'
  import CraftDetailSubTable from './subTables/CraftDetailSubTable'
  import {filterMultiDictText} from '@/components/dict/JDictSelectUtil'
  import '@/assets/less/TableExpand.less'
  import {getAction} from "@api/manage";

  export default {
    name: 'proConsignSettleList',
    mixins: [JeecgListMixin],
    components: {
      proConsignSettleModal,
      CraftDetailSubTable,
    },
    data() {
      return {
        ...
        expandedRowKeys: [],
        url: {
          list: '/bill/bill/list/procs',
          delete: '/bill/bill/delete',
          deleteBatch: '/bill/bill/deleteBatch',
          exportXlsUrl: '/bill/bill/exportXls/pcs',
