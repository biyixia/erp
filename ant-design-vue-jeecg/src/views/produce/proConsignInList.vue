<template>
  <a-card class="j-inner-table-wrapper" :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" icon="download" @click="handleExportXls('生产委托入库单')">导出</a-button>
...
<!--      <j-super-query :fieldList="superFieldList" ref="superQueryModal" @handleSuperQuery="handleSuperQuery"></j-super-query>-->
      <a-dropdown v-if="selectedRowKeys.length > 0">
        <a-menu slot="overlay">
          ...
          <a-menu-item key="2" v-has="'pci:check'" @click="batchSetStatus(1)"><a-icon type="check"/>审核</a-menu-item>
          <a-menu-item key="3" v-has="'pci:check'" @click="batchSetStatus(0)"><a-icon type="stop"/>反审核</a-menu-item>
        </a-menu>
        <a-button>
          <span>批量操作</span>
          <a-icon type="down"/>
      ...
      >
        <template slot="expandedRowRender" slot-scope="record">
          <a-tabs tabPosition="top">
            <a-tab-pane tab="委托入库明细" key="billingDetail" forceRender>
        ...
        <template slot="customRenderStatus" slot-scope="status">
          ...
          <a-tag v-if="status === '2'" color="cyan">部分结算</a-tag>
          <a-tag v-if="status === '3'" color="blue">全部结算</a-tag>
        </template>
    ...
    <pro-consign-in-modal ref="modalForm" @ok="modalFormOk"/>
  </a-card>
</template>
<script>

  import { JeecgListMixin } from '@/mixins/JeecgListMixin'
  import proConsignInModal from './modules/proConsignInModal'
  ...
  export default {
    ...
    data() {
      return {
        ...
        expandedRowKeys: [],
        url: {
          list: '/bill/bill/list/proci',
          delete: '/bill/bill/delete',
          deleteBatch: '/bill/bill/deleteBatch',
          exportXlsUrl: '/bill/bill/exportXls/pci',
