        ...
        <a-row :gutter="24">
          ...
          <a-col :md="6" :sm="24">
            <a-form-item label="未收款　" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
              <j-dict-select-tag type="list" v-model="queryParam.noCollect" dictCode="is_open"
                                 placeholder="是否未收款"/>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="24">
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          customRender(text, row, index) {
            return row.taxIncludedAmount - row.sumOutBound;
          }
        },
        {
          title: '创建人',
  ...
  methods: {
    async loadData(arg) {
      ...
      this.loading = true
      params.total = true
      getAction('/bill/bill/referPoList/si', params).then(res => {
        if (res.success) {
